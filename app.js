...
const SUPABASE_URL = typeof CONFIG !== 'undefined' ? CONFIG.SUPABASE_URL : 'https://pxozxarxtvbvswunmllz.supabase.co';
...
const p4 = 'bl9iR1u3';
const p5 = 'fO9zlTM';
const GEMINI_API_KEY = typeof CONFIG !== 'undefined' && CONFIG.GEMINI_API_KEY ? CONFIG.GEMINI_API_KEY : p1 + p2 + p3 + p4 + p5;
...
async function initializeApp() {
    ...
    if (session) {
        ...
        // Force the user to login or continue as guest manually.
...
function updateAuthUI() {
    ...
    // Select all main app sections by stable class names (not auth-required which gets removed)
    const appSections = document.querySelectorAll('.financial-profile, .goals-section, .lists-section, .budgets-section, .add-item-section, .filters-section, .wishlist-section, .whatif-section, .savings-planner');
    ...
    if (currentUser) {
        if (currentUser.email) {
            ...
            userEmailEl.textContent = 'Using Guest session (data not synced)';
            logoutBtn.style.display = 'inline-block';
            showLoginBtn.style.display = 'none';
        }
        loginForm.style.display = 'none';
        // Show app sections by removing the hiding class
        appSections.forEach(el => el.classList.remove('auth-required'));
    } else {
        userEmailEl.textContent = 'Please log in to view and plan your wishlist.';
        logoutBtn.style.display = 'none';
        showLoginBtn.style.display = 'inline-block';
        loginForm.style.display = 'block'; // Show login form by default
        // Hide app sections by adding the hiding class back
        appSections.forEach(el => el.classList.add('auth-required'));
    }
}
    ...
    if (demoLoginBtn) {
        demoLoginBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                alert('Error logging into demo account. Have you run the supabaseClient-demo-data.sql script yet? Error: ' + error.message);
            } finally {
...
async function saveFinancialProfile() {
    ...
    if (error) {
        ...
        if (error.message && error.message.includes('days_per_month')) {
            ...
            const { data, err: retryError } = await supabaseClient
                .from('financial_profiles')
...
async function scrapeProduct() {
    ...
    statusEl.textContent = '✨ Analyzing product with AI...';

    try {
        const prompt = `You are a product data extractor.
I will give you a product URL. I want you to extract as much information as possible from the URL itself (like the product name from the slug, the brand, etc.). 
Even if you cannot visit the webpage, you MUST infer the product name from the URL text.

URL: ${url}

Return ONLY a valid JSON object with EXACTLY these keys:
{
  "product_title": "The inferred product name from the URL slug (e.g., if URL has 'iphone-15-pro', title is 'iPhone 15 Pro'). Max 10 words.",
  "price": null,
  "main_image_url": null,
  "list_of_specifications": ["Inferred feature 1", "Inferred feature 2"],
  "suggested_category": "Inferred category (e.g. Electronics, Home, Beauty)",
  "tags": ["brand", "category", "keyword"]
}

Do not return anything except the JSON object. Do not explain your reasoning.`;
        ...
        document.getElementById('detailPurchaseLink').value = url;
        updateImagePreview('');
        ...
        document.getElementById('detailTags').value = Array.isArray(product.tags)
            ? product.tags.join(', ')
            : '';
        ...
        statusEl.textContent = '✅ Product analyzed! Please review the details, add a price and image if needed, then save.';
    } catch (error) {
        console.error('Error analyzing product:', error);
        statusEl.className = 'status-message error';
        const msg = (error.message || '').toLowerCase();
        if (msg.includes('overloaded') || msg.includes('quota') || msg.includes('rate limit')) {
            statusEl.textContent = '⚠️ AI is temporarily overloaded. Please wait a moment and try again.';
        } else {
            statusEl.textContent = '❌ Could not analyze product. Please use Manual Entry instead.';
        }
    }
}
...
async function checkItemPrice(item) {
    ...
    try {
        ...
        // Try using supabaseClient Edge Function first (skip if running from file:// protocol)
        const isFileProtocol = window.location.protocol === 'file:' || window.location.origin === 'null' || !window.location.origin;
        
        if (!isFileProtocol) {
            try {
                const supabaseProjectUrl = supabaseClient.supabaseUrl;
